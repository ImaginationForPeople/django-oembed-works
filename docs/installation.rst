...
django-oembed-works in your system. It also contains brief instructions about how
...
django-oembed-works distribution package. If ``pip`` is used to install this software,
...
You can download the latest django-oembed-works releases from the `releases page`_ at
the *Python Package Index* (PyPI).

.. _`releases page`: http://pypi.python.org/pypi/django-oembed-works
...
and then check-out any stable release, all of which are tagged::
    ...
    hg clone https://source.codetrax.org/hgroot/django-oembed-works
    hg tags
    hg update 0.1.0
...
To install django-oembed-works, use the provided installation script::

    python setup.py install

You can install ``django-oembed-works`` using ``pip``::

    pip install django-oembed-works

Or use ``easy_install``::

    easy_install -Z django-oembed-works

Note: the ``-Z`` flag is required to force ``easy_install`` to do a normal
source install rather than a zipped egg; django-oembed-works cannot be
used from a zipped egg install.

Alternatively, you can simply place the ``oembed_works`` directory,
...
the `source code repository`_ using ``pip``::
    ...
    pip install -e hg+https://source.codetrax.org/hgroot/django-oembed-works#egg=django-oembed-works
...
for instance, the ``0.1.0`` release, run the following command::
    ...
    pip install -e hg+https://source.codetrax.org/hgroot/django-oembed-works@0.1.0#egg=django-oembed-works-0.1.0

Please note that the mercurial_ source control management tool is required
for this operation.

.. _mercurial: http://mercurial.selenic.com/
.. _`source code repository`: https://source.codetrax.org/hgroot/django-oembed-works
